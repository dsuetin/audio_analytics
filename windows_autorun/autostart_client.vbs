' autostart_client.vbs
' Launches the client with the BASE (system) pythonw.exe directly from the
' Python instance that .venv was created from (the "home =" line in
' pyvenv.cfg). The venv's site-packages are exposed through PYTHONPATH so all
' dependencies (grpcio, numpy, sounddevice, aiokafka, ...) are loaded from the
' virtual environment. This way only ONE process appears per client instance
' in Task Manager (no venv-launcher wrapper).
...
sh.Run """" & py & """ """ & installDir & "\client.py""", 0, False
